...
public class VM_Settings : ReactiveObject, IDisposable, IActivatableViewModel
{
    ...
    public void SaveSettings()
    {
        ...
        if (_restoreComplete)
        {
            ...
            // still hold the pre-restore state — syncing them back would clobber the restored
            // file on exit. The app is restarting anyway.
            BsaContentsDiag.Log("SaveSettings EARLY RETURN — restore in progress (restored file kept)");
            return;
        }
        if (_environmentStateProvider.Status != EnvironmentStateProvider.EnvironmentStatus.Valid)
        {
