...
public class VM_Settings : ReactiveObject, IDisposable, IActivatableViewModel
{
        ...
        OutputValidator outputValidator)
    {
        ...
        RescanCachedModCommand.ThrownExceptions
            .Subscribe(ex => ScrollableMessageBox.ShowError(string.Format(GetTranslation("msg_failedToRescanMod", "Failed to re-scan mod: {0}"), ExceptionLogger.GetExceptionStack(ex))))
            .DisposeWith(_disposables);
        ...
        DeleteCachedFaceFinderImagesCommand.ThrownExceptions
            .Subscribe(ex => ScrollableMessageBox.ShowError(string.Format(GetTranslation("msg_errorDeletingCachedFaceFinderImages", "Error deleting cached FaceFinder images: {0}"), ExceptionLogger.GetExceptionStack(ex))))
            .DisposeWith(_disposables);
        ...
        DeleteAutoGeneratedMugshotsCommand.ThrownExceptions
            .Subscribe(ex => ScrollableMessageBox.ShowError(string.Format(GetTranslation("msg_errorDeletingAutoMugshots", "Error deleting auto-generated mugshots: {0}"), ExceptionLogger.GetExceptionStack(ex))))
            .DisposeWith(_disposables);
        ...
        BackupSettingsCommand = ReactiveCommand.CreateFromTask(BackupSettingsAsync).DisposeWith(_disposables);
                ...
                ExportDescriptionsCsvCommand.ThrownExceptions
                    .Subscribe(ex => ScrollableMessageBox.ShowError(string.Format(GetTranslation("msg_errorExportingDescriptionsCsv", "Error exporting descriptions CSV: {0}"), ExceptionLogger.GetExceptionStack(ex))))
                    .DisposeWith(_disposables);
                ImportTranslationsCsvCommand.ThrownExceptions
                    .Subscribe(ex => ScrollableMessageBox.ShowError(string.Format(GetTranslation("msg_errorImportingTranslationsCsv", "Error importing translations CSV: {0}"), ExceptionLogger.GetExceptionStack(ex))))
                    .DisposeWith(_disposables);
        BackupSettingsCommand.ThrownExceptions
            .Subscribe(ex => ScrollableMessageBox.ShowError(string.Format(GetTranslation("msg_errorBackingUpSettings", "Error backing up settings: {0}"), ExceptionLogger.GetExceptionStack(ex))))
            .DisposeWith(_disposables);
        RestoreSettingsCommand.ThrownExceptions
            .Subscribe(ex => ScrollableMessageBox.ShowError(string.Format(GetTranslation("msg_errorRestoringSettings", "Error restoring settings: {0}"), ExceptionLogger.GetExceptionStack(ex))))
            .DisposeWith(_disposables);
        GenerateAllMugshotsCommand.ThrownExceptions
            .Subscribe(ex => ScrollableMessageBox.ShowError(string.Format(GetTranslation("msg_errorGeneratingMugshots", "Error generating mugshots: {0}"), ExceptionLogger.GetExceptionStack(ex))))
            .DisposeWith(_disposables);
        ...
        BatchDownloadFaceFinderMugshotsCommand.ThrownExceptions
            .Subscribe(ex => ScrollableMessageBox.ShowError(string.Format(GetTranslation("msg_errorDownloadingFaceFinderMugshots", "Error downloading FaceFinder mugshots: {0}"), ExceptionLogger.GetExceptionStack(ex))))
            .DisposeWith(_disposables);
            ...
            .Subscribe(pair =>
            {
                ...
                else if (current == WigHandlingMode.ForwardToOutfit && UseSkyPatcherMode)
                {
                    warning = HandlingModeDisplay.SkyPatcherForwardToOutfitWarning +
                              GetTranslation("msg_useForwardToOutfitAnyway", "\n\nUse Forward To Outfit anyway?");
        ...
        this.WhenAnyValue(x => x.SelectedRecordOverrideHandlingMode)
            ...
            .Subscribe(mode =>
            {
                ...
                {
                    string message = GetTranslation("msg_warnOverrideHandlingMode",
            ...
            .Subscribe(useSkyPatcher =>
            {
                if (useSkyPatcher) // If the checkbox was just checked
                {
                    var message = new StringBuilder(
                        GetTranslation("msg_skypatcherWarning",
                    ...
                    message.Append(GetTranslation("msg_skypatcherConfirm", "\n\nAre you sure you want to use SkyPatcher Mode?"));

                    if (!SuppressPopupWarnings &&
                        !ScrollableMessageBox.Confirm(message.ToString(), GetTranslation("title_confirmSkyPatcherMode", "Confirm SkyPatcher Mode")))
                    {
        ...
        if (File.Exists(settingsPath))
        {
            ...
            if (!success || loadedSettings == null) // Add the null check here
            {
                ...
                ScrollableMessageBox.ShowWarning(
                    string.Format(GetTranslation("msg_settingsLoadError", "Error loading settings from {0}:\n{1}\n\nDefault settings will be used."), settingsPath, exception),
                    GetTranslation("title_settingsLoadError", "Settings Load Error"));
    ...
    private async Task ValidateOutputAsync()
    {
        ...
        {
            ScrollableMessageBox.ShowWarning(
                GetTranslation("msg_gameEnvironmentInvalid", "The game environment is not valid. Resolve it on this Settings page (a working load order and data folder are required) before validating output."), TranslationServiceProvider.GetService()?.GetString("validateOutput") ?? "Validate Output");
            return;
        }
        ...
        if (_outputWrittenThisSession && !_staleOutputWarningAcknowledged)
        {
            var proceed = ScrollableMessageBox.Confirm(
                GetTranslation("msg_staleOutputWarning",
    ...
    // --- Pre-translate all NPC descriptions into the local cache ---
        ...
        private async Task PreCacheDescriptionsAsync()
        {
            ...
            {
                ScrollableMessageBox.ShowWarning(
                    GetTranslation("msg_npcListNotInitialized", "The NPC list has not been initialized yet. Open the main window first, then retry."),
            ...
            catch (Exception ex)
            {
                ...
                ScrollableMessageBox.ShowError(
                    string.Format(GetTranslation("msg_preTranslationFailed", "Pre-translation failed: {0}"), ExceptionLogger.GetExceptionStack(ex)),
                    TranslationServiceProvider.GetService()?.GetString("preCacheDescriptions") ?? "Pre-translate NPC Descriptions");
                return;
            }
                ...
                private async Task RestoreSettingsAsync()
                {
                    ...
                    Application.Current.Shutdown();
                        ...
                        private async Task ExportDescriptionsCsvAsync()
                        {
                            ...
                            while (!finished)
                            {
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    progressVm.Dispose();
                                    ScrollableMessageBox.ShowError(
                                        string.Format(GetTranslation("msg_descriptionFetchFailed", "Description fetch failed:\n{0}"), ex.Message),
                                        exportTitle);
                                    return;
                                }
                                finally
                        ...
                        private async Task ImportTranslationsCsvAsync()
                        {
                            ...
                            {
                                ScrollableMessageBox.ShowError(
                                    string.Format(GetTranslation("msg_importFailed", "Import failed: {0}"), ExceptionLogger.GetExceptionStack(ex)),
                                    TranslationServiceProvider.GetService()?.GetString("importTranslations") ?? "Import Translations (CSV)");
                            }
                        }
    ...
    private async Task DeleteCachedFaceFinderImagesAsync()
    {
        ...
        try
        {
            ...
            messageBuilder.AppendLine("═══════════════════════════════════════════════════════════");
            messageBuilder.AppendLine();
            messageBuilder.AppendLine(GetTranslation("msg_areYouSureContinue", "Are you sure you want to continue?"));

            // Confirm deletion with scrollable list
            if (!ScrollableMessageBox.Confirm(messageBuilder.ToString(), GetTranslation("title_confirmDeletion", "Confirm Deletion")))
            {
                return;
            }
            ...
            if (progressVM.IsCancellationRequested)
            {
                ScrollableMessageBox.Show(string.Format(GetTranslation("msg_cachedFaceFinderDeletePartial", "Operation cancelled. Deleted {0} of {1} cached FaceFinder images."), deletedCount, cachedFiles.Count),
                    GetTranslation("title_deletionCancelled", "Deletion Cancelled"));
            }
            else
            {
                ...
                ScrollableMessageBox.Show(string.Format(GetTranslation("msg_deletedCachedFaceFinderImages", "Successfully deleted {0} cached FaceFinder image(s)."), deletedCount),
                    GetTranslation("title_deletionComplete", "Deletion Complete"));
            }
        }
        catch (Exception ex)
        {
            progressWindow?.Close();
            ScrollableMessageBox.ShowError(string.Format(GetTranslation("msg_anErrorOccurred", "An error occurred: {0}"), ExceptionLogger.GetExceptionStack(ex)));
        }
    }
    ...
    {
        string helpText = GetTranslation("helpPatchingMode",
            @"Patching Mode Help:
...
When the ouptut plugin is generated, put it at the end of your load order.");
        ...
        ScrollableMessageBox.Show(helpText, GetTranslation("title_patchingModeInfo", "Patching Mode Information"));
    }

    private void ShowOverrideHandlingModeHelp()
    {
        string helpText = GetTranslation("helpOverrideHandlingMode",
            @"Override Handling Mode:
...
- Include As New:
  ...
  - This mode may be appropriate if you have two Appearance Mods overriding the same record in different ways, which would make them incompatible with each other otherwise.");
        ...
        ScrollableMessageBox.Show(helpText, GetTranslation("title_overrideHandlingInfo", "Override Handling Mode Information"));
    }
...
// --- NEW: Method to launch the color picker dialog ---
    ...
    private async Task GenerateAllMugshotsAsync()
    {
        ...
        if (!generateForAllMods)
        {
            ...
            if (modSettings.Count == 0)
            {
                ScrollableMessageBox.Show(
                    string.Format(GetTranslation("msg_selectedModScopeNotFound", "The selected mod '{0}' was not found. It may have been removed or renamed; choose another option from the dropdown."), selectedModScope),
        ...
        {
            ScrollableMessageBox.ShowWarning(
                GetTranslation("msg_localRendererDisabled",
                ...
                GetTranslation("title_nothingToGenerate", "Nothing To Generate"));
            return;
        }
        ...
        {
            ScrollableMessageBox.ShowError(
                GetTranslation("msg_environmentNotReady", "The Skyrim load order isn't initialized; cannot resolve NPC records."),
                GetTranslation("title_environmentNotReady", "Environment Not Ready"));
            return;
        }
        ...
        summary.AppendLine(progressVM.IsCancellationRequested
            ? string.Format(GetTranslation("msg_generationAbortedAfter", "Mugshot generation aborted after {0} of {1} NPCs."), generatedCount + skippedCount + failedCount + missingAssetSkippedCount, workList.Count)
            : string.Format(GetTranslation("msg_generationCompleteFor", "Mugshot generation complete for {0} NPCs."), workList.Count));
        summary.AppendLine();
        summary.AppendLine(string.Format(GetTranslation("msg_summaryGenerated", "Generated:    {0}"), generatedCount));
        summary.AppendLine(string.Format(GetTranslation("msg_summaryAlreadyCurrent", "Already current: {0}"), skippedCount));
        if (missingAssetSkippedCount > 0)
        {
            summary.AppendLine(string.Format(GetTranslation("msg_summarySkippedMissingAssets", "Skipped (missing assets): {0}"), missingAssetSkippedCount));
        }
        summary.AppendLine(string.Format(GetTranslation("msg_summaryFailedNoSource", "Failed / no source: {0}"), failedCount));
        summary.AppendLine();
        summary.AppendLine(string.Format(GetTranslation("msg_summaryElapsed", "Elapsed: {0}"), FormatElapsed(stopwatch.Elapsed)));

        ScrollableMessageBox.Show(summary.ToString(),
            progressVM.IsCancellationRequested ? GetTranslation("title_generationAborted", "Generation Aborted") : GetTranslation("title_generationComplete", "Generation Complete"));
    }
    ...
    private async Task BatchDownloadFaceFinderMugshotsAsync()
    {
        ...
        {
            ScrollableMessageBox.ShowWarning(
                GetTranslation("msg_facefinderDisabled", "FaceFinder is disabled. Enable \"Use FaceFinder API for missing mugshots\" before running this batch."),
                GetTranslation("title_facefinderDisabled", "FaceFinder Disabled"));
            return;
        }
        ...
        if (!CacheFaceFinderImages)
        {
            var confirmNoCache = MessageBox.Show(
                GetTranslation("msg_facefinderCachingOff",
                ...
                GetTranslation("title_facefinderCachingDisabled", "FaceFinder Caching Disabled"),
        ...
        summary.AppendLine(progressVM.IsCancellationRequested
            ? string.Format(GetTranslation("msg_facefinderBatchAborted", "FaceFinder batch aborted after {0} of {1} NPCs."), totalProcessed, totalNpcsExpected)
            : string.Format(GetTranslation("msg_facefinderBatchComplete", "FaceFinder batch complete for {0} NPCs."), totalNpcsExpected));
        summary.AppendLine();
        summary.AppendLine(string.Format(GetTranslation("msg_summaryModsMatched", "Mods matched on server: {0}"), modsResolvedCount));
        summary.AppendLine(string.Format(GetTranslation("msg_summaryModsNotOnFaceFinder", "Mods not on FaceFinder: {0}"), modsNotOnServerCount));
        summary.AppendLine();
        summary.AppendLine(string.Format(GetTranslation("msg_summaryDownloaded", "Downloaded:          {0}"), downloadedCount));
        summary.AppendLine(string.Format(GetTranslation("msg_summaryAlreadyCached", "Already cached:      {0}"), alreadyCachedCount));
        if (availableCount > 0)
        {
            summary.AppendLine(string.Format(GetTranslation("msg_summaryAvailableNotCached", "Available (not cached): {0}"), availableCount));
        }
        summary.AppendLine(string.Format(GetTranslation("msg_summaryNotOnFaceFinder", "Not on FaceFinder:   {0}"), notAvailableCount));
        if (errorCount > 0)
        {
            summary.AppendLine(string.Format(GetTranslation("msg_summaryErrors", "Errors:              {0}"), errorCount));
        }
        summary.AppendLine();
        summary.AppendLine(string.Format(GetTranslation("msg_summaryElapsed", "Elapsed: {0}"), FormatElapsed(stopwatch.Elapsed)));

        ScrollableMessageBox.Show(summary.ToString(),
            progressVM.IsCancellationRequested ? GetTranslation("title_downloadAborted", "Download Aborted") : GetTranslation("title_downloadComplete", "Download Complete"));
    }
    ...
    private async Task DeleteAutoGeneratedMugshotsAsync()
    {
        ...
        try
        {
            ...
            // Build the detailed message
            var messageBuilder = new StringBuilder();
            messageBuilder.AppendLine(string.Format(GetTranslation("msg_autoGenMugshotsFound", "Found {0} auto-generated mugshot(s) in {1} folder(s)."), autoGenFiles.Count, filesByFolder.Count));
            messageBuilder.AppendLine();
            messageBuilder.AppendLine(GetTranslation("msg_permDeleteAllAutoGen", "This will permanently delete all auto-generated portrait images."));
            messageBuilder.AppendLine(GetTranslation("msg_emptyFoldersRemoved", "Empty folders will also be removed."));
            messageBuilder.AppendLine();
            messageBuilder.AppendLine(GetTranslation("msg_filesToBeDeleted", "Files to be deleted:"));
            messageBuilder.AppendLine("═══════════════════════════════════════════════════════════");
            messageBuilder.AppendLine();
            ...
            if (progressVM.IsCancellationRequested)
            {
                ScrollableMessageBox.Show(string.Format(GetTranslation("msg_deletedAutoGenMugshotsPartial", "Operation cancelled. Deleted {0} of {1} auto-generated mugshots."), deletedCount, autoGenFiles.Count),
                    GetTranslation("title_deletionCancelled", "Deletion Cancelled"));
            }
            else
            {
                ...
                ScrollableMessageBox.Show(string.Format(GetTranslation("msg_deletedAutoGenMugshots", "Successfully deleted {0} auto-generated mugshot(s)."), deletedCount),
                    GetTranslation("title_deletionComplete", "Deletion Complete"));
            }
        }
        catch (Exception ex)
    ...
    private async Task RescanCachedModAsync(string path)
    {
        ...
        {
            ScrollableMessageBox.ShowWarning(string.Format(GetTranslation("msg_pathNotFound", "The path '{0}' no longer exists and cannot be scanned."), path),
                GetTranslation("title_pathNotFound", "Path Not Found"));
            return;
        }
        ...
        if (wasReimported)
        {
            ...
            ScrollableMessageBox.Show(
                string.Format(GetTranslation("msg_rescanSucceeded", "Successfully re-imported '{0}' as an appearance mod. You can now find it in the Mods menu."), Path.GetFileName(path)),
                GetTranslation("title_reScanSuccessful", "Re-Scan Successful"));
        }
        else
        {
            // Now includes the specific failure reason (e.g., "No FaceGen files found")
            ScrollableMessageBox.Show(
                string.Format(GetTranslation("msg_rescanFailed", "The mod at '{0}' is still not recognized as an appearance mod.\nReason: {1}"), Path.GetFileName(path), failureReason),
                GetTranslation("title_reScanFailed", "Re-Scan Failed"));
        }
    }
