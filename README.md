...
* **Out-of-scope assets** *(notes)* — an asset the mod references resolves fine, but from your **data folder** (a loose file or another mod's archive) rather than the mod's own Corresponding Mod Folders, and it isn't base-game content. Nothing is broken today; it's a *keep-that-mod-activated* dependency — the same class the blue folder badge flags on mugshot tiles, except the scan has time the renderer doesn't, so it also **cross-references your Mods folder and names the installed mod(s) actually shipping each file** (the **Provided By** column). To make such an asset travel with the mod instead, add the supplying mod's folder to the mod entry's Corresponding Mod Folders.
